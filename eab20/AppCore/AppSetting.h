//
//  AppSetting.h
//  SmartLife
//
//  Created by zppro on 13-7-15.
//  Copyright (c) 2013年 zppro. All rights reserved.
//

#ifndef EAB2_AppSetting_h
#define EAB2_AppSetting_h

#define APP_SETTING_IS_ACTIVIATED_KEY @"is_activiated"
#define APP_SETTING_ACTIVIATE_BASE_URL_KEY @"activiate_base_url"
#define APP_SETTING_ACTIVIATED_MOBILE_KEY @"activiated_mobile"
#define APP_SETTING_ACTIVIATED_CONTACT_ID_KEY @"activiated_contact_id"
#define APP_SETTING_ACTIVIATED_CONTACT_NAME_KEY @"activiated_contact_name"

#define APP_SETTING_AUTH_BASE_URL_KEY @"auth_base_url"
#define APP_SETTING_AUTHENENTICATION_INTERFACE_TYPE_KEY @"authentication_interface_type"
#define APP_SETTING_AUTH_DU_KEY @"auth_du" //uuid reverse
#define APP_SETTING_AUTH_SDN_KEY @"auth_sdn" //securiy device no
#define APP_SETTING_AUTH_TOKEN_KEY @"auth_token" //token
#define APP_SETTING_API_URL_KEY @"api_url"

#define APP_SETTING_IS_SYNCED_KEY @"is_synced"
#endif
